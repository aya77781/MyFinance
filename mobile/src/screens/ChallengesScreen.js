    ...
    <>
      ...
      >
        <View style={styles.banner}>
          ...
          <Text style={styles.bannerValue}>{new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(totalSetAside)}</Text>
          <Text style={styles.bannerSub}>{items.filter((c) => c.status === 'active').length} challenge(s) en cours</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  bannerLabel: { color: colors.textOnHeroMuted, fontSize: 14, fontWeight: '600' },
  bannerValue: { color: '#fff', fontSize: 34, fontWeight: '800', marginTop: 4 },
  bannerSub: { color: colors.warning, fontSize: 13, fontWeight: '700', marginTop: 4 },
});
