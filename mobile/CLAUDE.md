...
| 2026-06-29 | **backend/defaults.js** | **P0 BUG** `DEFAULT_CATEGORIES` (depenses) n'avaient pas de `type` -> la colonne `categories.type` est NOT NULL -> **toute inscription partait en 500** (impossible de creer un compte). Ajout `type: 'expense'` sur les 9 categories. Verifie par test e2e (31 endpoints, 0 echec apres correction). |
| 2026-06-29 | backend/supabase/migrations/0002_sync_schema.sql (nouveau) | **P1** Le schema 0001 etait perime vs la prod (patchee a la main). Migration idempotente qui recree `challenge_missions` + colonnes `categories.planned`, `incomes.category_id`, `transactions.budget_month`, `challenges.period`, `opportunities.date/paid_at`, et elargit le CHECK statut a `'paid'`. Rend un deploiement neuf reproductible. |
| 2026-06-29 | screens/Dashboard (gauge `balance` maxWidth + `Flow` flex/adjustsFontSizeToFit) | **P1 display** Gros montants : la jauge ne deborde plus (largeur bornee -> `adjustsFontSizeToFit` s'active), les flux Entrees/Sorties ne debordent plus de leur moitie. |
| 2026-06-29 | screens/Budget, Savings, Challenges, Opportunities | **P2 display** Valeurs "hero/banner" (38px) + cumul Budget (30px) : `numberOfLines={1} adjustsFontSizeToFit` -> plus de retour a la ligne / coupure sur 5-6 chiffres. |
| 2026-06-29 | components/FormSheet.js | **P2 display** `paddingBottom` integre `insets.bottom` (safe area) -> les boutons Valider/Supprimer ne passent plus sous le home indicator iPhone. Concerne tous les flux creation/edition. |
| 2026-06-29 | screens/DashboardScreen.js | **P2** Navigation entre mois sur "Solde du mois" : fleches ‹ › autour du libelle, etat `sel`, `load(period)` passe year/month a `Dashboard.get` (API+backend supportaient deja). Mois passes ET futurs (preparer les mois a venir). Tout le dashboard suit le mois choisi. |
| 2026-06-29 | screens/ChallengesScreen.js | **P2** Pistes desormais entierement modifiables : la feuille (ex-"Valider") edite intitule + montant estime + montant reel. Reel vide = simple modif ; reel renseigne = validation. Backend `PUT /missions/:id` supportait deja title/estimatedAmount (non expose avant). |
| 2026-06-29 | screens/ChallengesScreen.js | **P2** Total des pistes affiche en petit en bas de chaque carte defi (potentiel estime + "Gagne" cumule des pistes validees). |
| 2026-06-29 | components/GoalCard.js + screens/SavingsScreen.js | **P2** Edition des pochettes d'epargne (nom/objectif/couleur) : bouton "dots" sur GoalCard -> feuille d'edition + suppression. `Savings.update` (API) existait, non expose avant. |
| 2026-06-29 | screens/BudgetScreen.js | **P2** Revenus stables et charges fixes desormais modifiables (avant : suppression seule par appui long). Tap sur la ligne -> feuille de creation reutilisee en mode edition (titre dynamique, valeurs pre-remplies, bouton supprimer). `Income.update`/`Charges.update` existaient. |
| 2026-06-30 | backend/routes/savings.js, dashboard.js · mobile/Savings,GoalCard,FormSheet,api | **P2 Epargne refonte** Un versement date du mois courant est **deduit de l'argent disponible du mois** (dashboard : `savedThisMonth` -> `totalOut` -> solde net). Versement avec **date optionnelle** (defaut aujourd'hui) + rappel UX. **Versements modifiables/supprimables** : routes `PUT`/`DELETE /savings/:id/contributions/:cid` (recalcul auto de `currentAmount` = somme des versements, auto-correctif). GoalCard depliable -> historique des versements (tap = edition). FormSheet : props `hint` (champ) + `footnote` (feuille). |

**Verifie cette session (sante P0)** : **test e2e complet** (script jetable) — register + 30 endpoints (categories/income/charges/transactions/budgetMonth/savings/challenges+missions/opportunities paid/market) = **0 echec** contre la vraie base Supabase apres le fix d'inscription. **Le schema live a TOUTES les colonnes** (sonde directe : `challenge_missions`, `planned`, `category_id`, `budget_month`, `period`, `date`, `paid_at`, statut `paid` OK) — la prod n'est PAS cassee, seul l'onboarding l'etait. Tous les JSX touches passent `@babel/parser`.
